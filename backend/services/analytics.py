...
def _item(s: TestSession, rep: Optional[Report]) -> dict:
    ...
    return {
        ...
        "marginal_results": s.marginal_results,
        "rounding_traps": s.rounding_traps,
    }
...
def search_reports(
    ...
    if user.role == TESTER:
        # testers: everything approved + their own work
        q = q.filter(
            or_(
                TestSession.status == APPROVED,
                TestSession.tester_id == user.id,
            )
        )
    ...
    if search:
        ...
        q = q.filter(
            or_(
                ...
                Instrument.serial_no.ilike(like),
                Manufacturer.name.ilike(like),
                Report.report_no.ilike(like),
            )
        )
    ...
    if model:
        q = q.filter(
            Instrument.model.ilike(f"%{model}%")
        )
    ...
    if verdict:
        q = q.filter(
            TestSession.verdict_overall == verdict.upper()
        )

    if date_from:
        q = q.filter(
            TestSession.tested_at >= datetime.fromisoformat(date_from)
        )

    if date_to:
        q = q.filter(
            TestSession.tested_at
...
def instrument_history(
    ...
    rows = (
        ...
        .outerjoin(
            Report,
            Report.session_id == TestSession.id,
        )
        .filter(
            TestSession.instrument_id == instrument_id
        )
        .order_by(
            TestSession.tested_at.desc(),
            TestSession.id.desc(),
    ...
    return [
        {
            ...
            "status": s.status,
            "verdict": s.verdict_overall,
            "report_no": r.report_no if r else None,
        }
        for s, r in rows
    ]
...
def dashboard_stats(db: Session) -> dict:
    ...
    def count(statuses):
        return (
            ...
            .filter(TestSession.status.in_(statuses))
            .count()
        )
    ...
    approved_pass = (
        ...
        .filter(
            TestSession.status == APPROVED,
            TestSession.verdict_overall == "PASS",
        )
        .count()
    )
    ...
    approved_fail = (
        ...
        .filter(
            TestSession.status == APPROVED,
            TestSession.verdict_overall == "FAIL",
        )
        .count()
    )
    ...
    events = (
        db.query(AuditLog)
        .filter(AuditLog.description.isnot(None))
        .order_by(
            AuditLog.at.desc(),
            AuditLog.id.desc(),
        )
...
def failure_insights(db: Session) -> dict:
        ...
        db.query(TestSession)
        .filter(
            TestSession.verdict_overall.in_(["PASS", "FAIL"])
        )
        .all()
    )
    ...
    for r in (
        db.query(Result)
        .filter(Result.session_id.in_(ids))
        .all()
        if ids
        else []
    ):
    ...
    for s in done:
        ...
        m = by_mfr.setdefault(
            s.instrument.manufacturer.name,
            [0, 0],
        )
